...
started = True
def log_error(e):
    log_path = "logs/remove_stopping_frames"
    log_name = "errors_%i.log"

    import re
    logs = os.listdir(log_path)
    max_log_number = max([re.findall(r"\d+", log) for log in logs]) if len(logs) > 0 else 0

    global started
    if not started:
        max_log_number += 1
        import datetime
        with open(os.path.join(log_path, log_name % max_log_number), "w") as f:
            f.write("%s\n" % str(datetime.now()))
        started = True
